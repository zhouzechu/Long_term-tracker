...
from global_search import glob_redect, crop_template_Hao
...
class SiamFCppLTTracker(PipelineBase):
    ...
    def init(self, im, state):
        ...
        temp_redect = crop_template_Hao(im.astype('uint8'), rect)
        box = xywh2cxywh(rect)
        target_pos, target_sz = box[:2], box[2:]
        self._state['temp_crop'] = temp_redect
        self._state['im_h'] = im.shape[0]
        self._state['im_w'] = im.shape[1]
        self._state['template_size'] = target_sz
    ...
    def track(self,
        ...
        # 此处对于pscore[best_pscore_id]做一个判断，如果大于0.3则认为目标存在，否则进入全局搜索
        if pscore[best_pscore_id] > 0.3:
            # box post-processing
            temp_exist = 1
            ...
            if update_state:
                ...
                self._state['cls'] = cls
                self._state['ctr'] = ctr
            # print("spend time3:", time.time()-time_start3)

            return new_target_pos, new_target_sz, temp_exist

        if pscore[best_pscore_id] <= 0.3:
            im_x = im_x.astype('uint8')
            temp_exist = 0
            w, h = self._state['template_size']
            im_z = self._state['temp_crop']
            if pscore[best_pscore_id] < self._hyper_params["lost_score_th_low"]:
                self._state["lost_target"] = True
                logger.info("lost target")
            center_pos, _ = glob_redect(im_x, (w, h), im_z)
            new_target_pos = center_pos
            new_w = w
            new_h = h
            new_target_sz = np.array([new_w, new_h])
            return new_target_pos, new_target_sz, temp_exist
    ...
    def update(self, im, flag, state=None):
        ...
        target_pos, target_sz, flag = self.track(im,
                                           target_pos_prior,
                                           target_sz_prior,
                                           features,
                                           flag,
